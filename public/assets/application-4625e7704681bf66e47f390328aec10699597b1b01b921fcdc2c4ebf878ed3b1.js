...
(function() {
  ...
  Email = {
    init: function() {
      $('body').on('ajax:success', '#password-reset, .send-email', this.showSent);
      return $('body').on('submit', '.send-email', this.showSending);
    },
    showSent: function(e, data) {
    ...
    showSending: function(e, data) {
      return $('.form-message').attr('style', '').show().text("Sending...");
    }
  };
...
(function() {
  ...
  Header = {
    init: function() {
      $('body').on('click', '#close-dropdown, #menu-icon', this.toggleMenuOnClick);
      $('body').on('mouseenter', '#nav-profile-pic', this.showProfileDropdown);
      $('body').on('mouseleave', '#profile-dropdown', this.closeProfileDropdown);
      $('body').on('mouseenter', '#create-btn', this.showCreationDropdown);
      $('body').on('mouseleave', '.creation-dropdown', this.closeCreationDropdown);
      $('body').on('mouseenter', '.fun-dropdown-trigger', this.showFunDropdown);
      $('body').on('mouseleave', '.fun-dropdown', this.closeFunDropdown);
      $('body').on('click', '#search-icon', this.openSearch);
      return $('body').on('mouseenter', '#get-started', this.showCreationDropdown);
    },
    openSearch: function() {
      $('.search-input').toggle();
      return $('#logo').toggle();
    ...
    showCreationDropdown: function() {
      ...
      return check = setInterval(function() {
        if ($('.creation-dropdown:hover').length < 1 && $('#create-btn:hover').length < 1 && $('.bridge:hover').length < 1 && $('#get-started:hover').length < 1) {
          return btn.parent().find('.creation-dropdown').removeClass('shown-flexed');
        }
      }, 500);
    },
    closeCreationDropdown: function() {
      return $('.creation-dropdown').removeClass('shown-flexed');
    },
    showFunDropdown: function() {
      var check;
      $('.fun-dropdown').show();
      return check = setInterval(function() {
        if ($('.fun-dropdown:hover').length < 1 && $('.fun-dropdown-trigger:hover').length < 1 && $('.bridge:hover').length < 1) {
          return $('.fun-dropdown').hide();
        }
      }, 1000);
    },
    closeFunDropdown: function() {
      return $('.fun-dropdown').hide();
    }
  };
...
(function() {
  ...
  PQuizShow = {
    init: function() {
      $('body').on('click', '.starter-btn', this.startPQuiz);
      $('body').on('click', '.content-container.quiz .answer', this.answerPQuiz);
      return $('body').on('click', '.nav-back', this.navBack);
    },
    navBack: function() {
      var current, prev;
    ...
    startPQuiz: function() {
      ...
      return $(this).parents('.quiz-starter').animate({
        opacity: 0
      }, 400, function() {
        $(this).hide();
        $('.item').first().show().animate({
          opacity: 1
        }, 400);
        return $('.quiz-navigator').removeClass('hidden').animate({
          opacity: 1
        }, 400);
      });
    },
    answerPQuiz: function() {
      var current, currentNum, next;
...
(function() {
  var FlipcardNew, ready;

  FlipcardNew = {
    init: function() {
      $('body').on('click', '.color', this.chooseColor);
      $('body').on('click', '.add-card-text', this.addCardText);
      $('body').on('click', '.side-chooser', this.chooseSide);
      $('body').on('keyup', '.text-box', this.syncItemText);
      return this.centerCardTexts();
    },
    centerCardTexts: function() {
      return $('.text-box').each(function() {
        var height, offset;
        height = $(this).height();
        offset = (260 - height) / 2;
        return $(this).css('top', offset);
      });
    },
    chooseSide: function() {
      var card;
      card = $(this).parents('.question');
      card.find('.side-chooser').removeClass('selected');
      $(this).addClass('selected');
      if ($(this).hasClass('choose-front')) {
        return card.find('.flip-container').removeClass('flip');
      } else {
        return card.find('.flip-container').addClass('flip');
      }
    },
    addCardText: function() {
      var card;
      card = $(this).parents('.question-image-input');
      card.find('.text-box')[0].contentEditable = true;
      return card.find('.text-box').focus();
    },
    syncItemText: function() {
      var card, height, offset;
      card = $(this).parents('.question-image-input');
      card.find('.card-text-input').val($(this).text());
      height = $(this).height();
      offset = (260 - height) / 2;
      return $(this).css('top', offset);
    },
    chooseColor: function() {
      ...
      return card.find('.card-color-input').val(color);
    }
  };

  ready = function() {
    return FlipcardNew.init();
  };
...
(function() {
  var QuizImageUpload, ready;

  QuizImageUpload = {
    init: function() {
      $('body').on('change', "input[type='file']", this.showImagePreview);
      $('body').on('click', '.close-image', this.closeImage);
      $('body').on('click', "input[type='file']", this.updateLastselectedImage);
      return this.lastSelectedImage = null;
    },
    updateLastselectedImage: function() {
      return QuizImageUpload.lastSelectedImage = $(this).val();
    },
    showImagePreview: function() {
      var file, formData, height, img, input, previewBox, uploadForm;
      input = this;
      ...
      return $.ajax({
        ...
        var fileName, imageKey, imageKeyBack, modelId, modelName;
        ...
        return $.post('/image_keys', {
          ...
          if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
              ...
              return previewBox.attr('src', e.target.result);
            };
            reader.readAsDataURL(input.files[0]);
            $(input).parent().parent().find('.close-image').show();
            $(input).parents('.card-side').find('.card-color-input').val("");
            return $(input).parents('.card-side').find('.question-image-input').attr('style', "").css('background', 'none');
          }
        });
      });
    },
    closeImage: function() {
      var box, modelId, modelName, side;
      box = $(this).parent();
      ...
      modelId = $(this).parent().find('.image-form').attr('model_id');
      modelName = $(this).parent().find('.image-form').attr('model_name');
      side = "front";
      $(this).parents('.card-side').find('div.question-image-input').css('background', '#1e5efd');
      if ($(this).parents('.card-side').length > 0 && $(this).parents('.card-side').hasClass('back-side')) {
        side = "back";
      }
      $.ajax({
...
(function() {
  ...
  QuizNew = {
    init: function() {
      this.questionCount = $('.question').length;
      $('body').on('click', '#add-question', this.addQuestion);
      $('body').on('click', '.close-question', this.closeQuestion);
      $('body').on('click', '.add-answer', this.addAnswer);
      $('body').on('click', '.close-answer', this.closeAnswer);
      $('body').on('click', '.add-result', this.addResult);
      $('body').on('click', '.close-result', this.closeResult);
      $('body').on('click', '.move-question', this.moveQuestion);
      $('body').on('click', '.answer-format', this.selectAnswerFormat);
      $('body').on('keyup', '.char-countable input, .char-countable textarea', this.showCharsRemaining);
      $('body').on('keypress', '.text-box', this.showCharsRemainingForTextbox);
      if ($('#new_quiz').length > 0) {
        window.onbeforeunload = this.confirmCloseWindow;
      }
      ...
      this.setAnswerFormats();
      return this.syncResultsRange();
    },
    closeSaveMessage: function() {
      return setTimeout(function() {
        return $('#flash-message').animate({
          opacity: 0
        }, 400);
      }, 2500);
    },
    confirmCloseWindow: function(e) {
      if ($('#new_quiz').length > 0) {
        e = e || window.event;
        if (e) {
          e.returnValue = 'Sure?';
    ...
    showImagePreviews: function() {
      $('.image-preview').show();
      return $(".image-preview[src='blank'], .image-preview[src='']").hide();
    },
    showCharsRemainingForTextbox: function(e) {
      var currentChars, limit, remaining;
      currentChars = $(this).text().trim().length;
      limit = parseInt($(this).attr('maxlength'));
      remaining = limit - currentChars;
      $(this).parent().find('.char-count').text(remaining);
      if (remaining === 0) {
        return e.preventDefault();
      }
    },
    showCharsRemaining: function() {
      ...
      limit = parseInt($(this).attr('maxlength'));
      remaining = limit - currentChars;
      return $(this).parent().find('.char-count').text(remaining);
    },
    syncResultsRange: function() {
      var i, j, k, questionLimit, questionsCount, range, rangeEnd, rangeStart, rangeWidth, ranges, ref, ref1, resultsCount;
      ...
      rangeStart = 0;
      rangeEnd = rangeWidth;
      for (i = j = 1, ref = resultsCount; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
        range = [];
      ...
      for (i = k = 1, ref1 = ranges.length; 1 <= ref1 ? k <= ref1 : k >= ref1; i = 1 <= ref1 ? ++k : --k) {
      ...
      return $('.form-result:visible .results-range').each(function(i) {
        console.log($(this));
        $(this).find('.min').text(ranges[i][0]);
        $(this).find('.max').text(ranges[i][1]);
        $(this).find('.range-min').val(ranges[i][0]);
        $(this).find('.range-max').val(ranges[i][1]);
        if (ranges[i][0] === ranges[i][1]) {
          return $(this).find('.min, .hyphen').hide();
        } else {
          return $(this).find('.min, .hyphen').show();
        }
      });
    },
    moveQuestion: function() {
      ...
      $('.question:visible .question-number').each(function(i) {
        return $(this).find('p').text("" + (i + 1));
      });
      return QuizNew.manageMovers();
    },
    manageMovers: function() {
      $('.move-question').show();
      $('.question:visible').first().find('.move-up').hide();
      return $('.question:visible').last().find('.move-down').hide();
    },
    addResultChoiceToAnswers: function() {
      return $('select').each(function() {
        var optionCount;
        optionCount = $(this).find('option').length;
        return $(this).append("<option value='" + (optionCount - 1) + "' class='result-" + optionCount + "'>Result " + optionCount + "</option>");
      });
    },
    addResult: function() {
      var lastResult, quizId, quizType;
      lastResult = $('.form-result:visible').last();
      quizId = $('.edit_quiz').attr('quiz_id');
      quizType = $('#quiz_type').val();
      return $.post("/quizzes/" + quizId + "/results?i=" + ($('.form-result').length) + "&quiz_type=" + quizType, function(data) {
        $('.form-result:visible').last().after(data);
        $('.close-result').show();
        if ($('.quiz-oriented-quiz').length > 0) {
          QuizNew.addResultChoiceToAnswers();
        }
        return QuizNew.syncResultsRange();
      });
    },
    ...
    removeResultChoice: function(index) {
      $('select').each(function() {
        if (parseInt($(this).val()) === index) {
          $(this).val(-1);
        }
        return $($(this).find('option')[index + 1]).remove();
      });
      return $('select').each(function() {
    ...
    addAnswer: function() {
      var i, itemNumber, lastAnswer, question, quizItemId, quizType;
      question = $(this).parents('.question');
      ...
      return $.post("/quiz_items/" + quizItemId + "/answers?item_number=" + itemNumber + "&i=" + i + "&quiz_type=" + quizType, function(data) {
        lastAnswer.after(data);
        if (i === 4 && $('.trivia-oriented-quiz').length > 0) {
          $(this).hide();
        }
        if (i === 8 && $('.quiz-oriented-quiz').length > 0) {
          $(this).hide();
        }
        return QuizNew.manageClosers();
      });
    },
    ...
    manageClosers: function() {
      ...
      $('.question:visible').each(function() {
        if ($(this).find('.form-answer:visible, .text-answer-form:visible').length < 2) {
          return $(this).find('.close-answer').hide();
        } else {
          return $(this).find('.close-answer').show();
        }
      });
      $('.close-image').each(function() {
        var imgPreview;
        imgPreview = $(this).parent().children('.image-preview');
        if (imgPreview.css('background-image').indexOf("amazonaws") >= 0) {
          return $(this).show();
        }
      });
      if ($('.form-result:visible').length < 2) {
        return $('.close-result').hide();
      } else {
        return $('.close-result').show();
      }
    },
    ...
    addQuestion: function() {
      ...
      quizId = $('.edit_quiz').attr('quiz_id');
      quizType = $('#quiz_type').val();
      return $.post("/quizzes/" + quizId + "/quiz_items?i=" + ($('.question').length) + "&quiz_type=" + quizType, function(data) {
    ...
    selectAnswerFormat: function() {
      var form, selected, type;
      type = $(this).attr('format_type');
      $(this).parent().find('.selected').hide();
      $(this).parent().find(':not(.selected)').show();
      selected = $(this).parent().find("[format_type='" + type + "']:hidden");
      $(this).hide();
      $(this).parent().find("[format_type='" + type + "']").hide();
      selected.show();
      form = $(this).parents('.form-box').find('.form-answers');
      if ($(this).parent().find('.image-format.selected:visible').length > 0) {
        form.addClass('image-style').removeClass('text-style');
      } else {
        form.removeClass('image-style').addClass('text-style');
      }
      return $(this).parents('.question').find('.answer-format').val($(this).attr('format_type'));
    },
    setAnswerFormats: function() {
      return $('input.answer-format').each(function() {
        if ($(this).val() === "image") {
          return $(this).parents('.question').find('.image-format').click();
        } else {
          return $(this).parents('.question').find('.text-format').click();
        }
      });
    }
  };
...
(function() {
  var QuizValidate, ready;

  QuizValidate = {
    init: function() {
      if ($('.flipcard-oriented-quiz').length < 1) {
        $('body').on('submit', '#new_quiz, .edit_quiz', this.validateFields);
      }
      return $('body').on('click', '#submit-bar .btn, .submittable', this.submitQuiz);
    },
    formatFieldNames: function() {
      $('.question').each(function(i) {
        console.log("Question " + i);
        $(this).find('input, checkbox, textarea, select').each(function() {
          var formattedName;
          formattedName = $(this).attr('name').replace(/quiz_item_\d*/, "quiz_item_" + (i + 1));
          return $(this).attr('name', formattedName);
        });
        return $('.question .form-answers').find('.form-answer').each(function(j) {
          console.log("Answer " + j);
          return $(this).find('input, checkbox, textarea, select').each(function() {
            var formattedName;
            formattedName = $(this).attr('name').replace(/item_answer_\d*/, "item_answer_" + (j + 1));
            return $(this).attr('name', formattedName);
          });
        });
      });
      return $('.form-result').each(function(i) {
        return $(this).find('input, checkbox, textarea, select').each(function() {
          var formattedName;
          formattedName = $(this).attr('name').replace(/result_\d*/, "result_" + (i + 1));
          return $(this).attr('name', formattedName);
        });
      });
    },
    validateFields: function() {
      var correctAnswersCount, description, flipCardError, i, image, imageBack, imageFront, incorrectNumberofAnswers, k, listItemError, missingFieldError, question, quizDescription, quizImage, quizTitle, ref, text, textBack, textFront;
      if ($('.quiz-submit-action').val() === "save-quiz") {
        return true;
      }
      ...
      if (!quizTitle || !quizDescription || !quizImage) {
        missingFieldError = true;
      }
      $('.question:visible .item-order').each(function(i) {
        return $(this).val(i + 1);
      });
      for (i = k = 0, ref = $('.question').length - 1; 0 <= ref ? k <= ref : k >= ref; i = 0 <= ref ? ++k : --k) {
        question = $($('.question')[i]);
        ...
        if (text.val() === "" && image.attr('src') === "blank") {
          if ($(".list-quiz").length < 1) {
            ...
            if (image.attr('src') === "blank") {
              question.find('div.question-image-input').css('border', '1px solid red');
            }
            missingFieldError = true;
          }
        } else {
          text.attr('style', '');
          if ($('.flipcard-quiz').length < 1) {
        ...
        question.find('.form-answer').each(function() {
          if ($(this).find("input[type='checkbox']:checked").length > 0) {
            correctAnswersCount += 1;
          }
          image = $(this).find('.answer-image-preview');
          text = $(this).find('.answer-text-input');
          if ($(this).find('.image-input:visible').length > 0) {
            if (image.attr('src') === "blank" && text.val() === "") {
              if (text.val() === "") {
                text.css('border', '1px solid red');
              }
              if (image.attr('src') === "blank") {
                $(this).find('.image-input').css('border', '1px solid red');
              }
              missingFieldError = true;
            } else {
              text.attr('style', '');
              $(this).find('.image-input').attr('style', '');
            }
          } else {
            if (text.val() === "") {
              text.css('border', '1px solid red');
          ...
          if ($(this).find('select').length > 0) {
            if ($(this).find('select').val() === null) {
              missingFieldError = true;
              return $(this).find('select').css('border', '1px solid red');
            } else {
              return $(this).find('select').attr('style', "");
            }
          }
        });
      ...
      $('.form-result').each(function() {
        text = $(this).find('.result-text-input');
        image = $(this).find('.result-image-preview');
        if (text.val() === "" && image.attr('src') === "blank") {
          if (text.val() === "") {
            text.css('border', '1px solid red');
          }
          ...
          return missingFieldError = true;
        } else {
          text.attr('style', '');
          return $(this).find('.image-input').attr('style', '');
        }
      });
    ...
    submitQuiz: function() {
      var action, actionText, formData, uploadForm, valid;
      ...
      actionText = $(this).attr('action-text');
      if (action === 'save-quiz') {
      ...
      return $('#new_quiz, .edit_quiz').submit();
    }
  };

  ready = function() {
    return QuizValidate.init();
  };
...
(function() {
  ...
  QuizIndex = {
    init: function() {
      return $('body').on('click', '#load-more', this.loadMore);
    },
    loadMore: function() {
      var offset;
      offset = $('.quiz-list-item').length;
      return $.get("/quizzes/featured?offset=" + offset, function(data) {
...
(function() {
  ...
  QuizShow = {
    init: function() {
      this.score = 0;
      this.resultScores = {};
      $('body').on('click', '.trivia .answer:not(.unselectable)', this.chooseTriviaAnswer);
      $('body').on('click', '.quiz .answer', this.chooseQuizAnswer);
      $('body').on('click', '.quiz-take .flip-container', this.flipCard);
      if ($(".list").length > 0) {
        return this.formatListImages();
      }
    },
    formatListImages: function() {
      return $('.item-image').each(function() {
        if ($(this).width() === 620) {
          $(this).css('width', "620px");
        }
        if ($(this).height() === 620) {
          return $(this).css('height', "620px");
        }
      });
    },
    flipCard: function() {
      return $(this).toggleClass('flip');
    },
    chooseQuizAnswer: function() {
      var answers, currentResultId;
      answers = $(this).parent();
      currentResultId = parseInt($(this).attr('result-id'));
      answers.find('.answer').each(function() {
        var resultId;
        resultId = parseInt($(this).attr('result-id'));
        if (QuizShow.resultScores[resultId]) {
          if ($(this).hasClass('selected')) {
            $(this).removeClass('selected');
            return QuizShow.resultScores[resultId] -= 1;
          }
        } else {
          return QuizShow.resultScores[resultId] = 0;
        }
      });
      QuizShow.resultScores[currentResultId] += 1;
      $(this).addClass('selected');
      $(this).parents('.item').addClass('answered');
      return QuizShow.checkForResultDisplay();
    },
    checkForResultDisplay: function() {
      if ($('.item:not(.answered)').length < 1 && $('.result:visible').length < 1) {
        return setTimeout(function() {
          return $(".result[result-id='" + (QuizShow.currentHighestResult()) + "']").show().css('opacity', 0).animate({
            opacity: 1
          }, 400);
        }, 400);
      }
    },
    currentHighestResult: function() {
      return Object.keys(QuizShow.resultScores).reduce(function(a, b) {
    ...
    chooseTriviaAnswer: function() {
      var answers;
      answers = $(this).parent();
      if ($(this).hasClass('correct') && !$(this).hasClass('unselectable')) {
        QuizShow.score += 1;
      } else {
    ...
    showResult: function() {
      ...
      $('.result').each(function() {
        var result;
        if (score >= parseInt($(this).attr('range-min')) && score <= parseInt($(this).attr('range-max'))) {
          $(this).show();
          result = $(this);
      ...
      return $('.score-number').text(score);
    }
  };

  ready = function() {
    return QuizShow.init();
  };
  ...
  QuizSort = {
    init: function() {
      if ($('.user-profile-container').length > 0) {
        return $('body').on('click', '.sort-bar p:not(.label)', this.chooseSortOption);
      }
    },
    chooseSortOption: function() {
      var i, j, quizzes, ref, results;
      $(this).parents().find('p').removeClass('selected');
      $(this).addClass('selected');
      quizzes = $('.sortable .sort-item');
      quizzes.sort(QuizSort.sortByOption($(this).attr('sort-name')));
      results = [];
      for (i = j = 0, ref = quizzes.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
        console.log(i);
        results.push(quizzes[i].parentNode.appendChild(quizzes[i]));
      }
      return results;
    },
    sortByOption: function(sortName) {
      if (sortName === "pd") {
        return QuizSort.sortByPublishDate;
      } else if (sortName === "cd") {
        return QuizSort.sortByCreationDate;
      } else {
        return QuizSort.sortByName;
      }
    },
    sortByPublishDate: function(a, b) {
      return $(b).attr('publish_date').localeCompare($(a).attr('publish_date'));
    },
    sortByCreationDate: function(a, b) {
      return $(b).attr('creation_date').localeCompare($(a).attr('creation_date'));
    },
...
(function() {
  var Pinterest, SocialMedia, ready;

  SocialMedia = {
    init: function() {
      $('body').on('click', '.social-media.fb', this.shareFb);
      $('body').on('click', '.social-media.tw', this.shareTw);
      return $('body').on('click', '.link', this.showLink);
    },
    showLink: function() {
      var linkBox;
      linkBox = $(this).parents('.social-medias').find('.link-box').show();
      return linkBox.find('input').select();
    },
    shareFb: function() {
      return FB.ui({
        method: 'share',
    ...
    shareTw: function() {
      ...
      return window.open("https://twitter.com/intent/tweet?text=" + text, 'Tweet!', 'height=600px,width=800px');
    },
    sharePinterest: function() {
      var currentUrl, description, fullUrl, media;
      ...
      return window.open(fullUrl, 'Pin it!', 'height=600px,width=800px');
    }
  };
