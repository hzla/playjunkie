...
(function() {
  ...
  QuizNew = {
    init: function() {
      this.questionCount = $('.question:visible').length;
    ...
    showImagePreviews: function() {
      $('.image-preview').show();
      return $(".image-preview[src='#'], .image-preview[src='']").hide();
    },
    ...
    addResult: function() {
      var incrementedResult, lastResult, newResult, resultCount;
      lastResult = $('.form-result:visible').last();
      newResult = lastResult.clone();
      resultCount = $('.form-result:visible').length;
      incrementedResult = newResult.html().replace(RegExp("result_" + resultCount, "g"), "result_" + (resultCount + 1));
    ...
    closeResult: function() {
      var form, result, resultNumber;
      form = $(this).parents('.new_quiz');
      result = $(this).parent();
    ...
    closeAnswer: function() {
      var answer, question;
      question = $(this).parents('.question');
      answer = $(this).parent().addClass('hidden').prependTo(question);
    ...
    closeQuestion: function() {
      ...
      if (confirmation) {
        $(this).parent().hide();
        $(this).parent().find('.box-input, .card-text-input').val("skip");
        $('.question:visible .question-number').each(function(i) {
          return $(this).find('p').text("" + (i + 1));
        });
    ...
    addQuestion: function() {
      var lastQuestion, newAnswer, newQuestion;
      lastQuestion = $('.question:visible').last();
      newQuestion = lastQuestion.clone();
      newQuestion = newQuestion.html().replace(RegExp("quiz_item_" + QuizNew.questionCount, "g"), "quiz_item_" + (QuizNew.questionCount + 1)).replace("<p>" + QuizNew.questionCount + "</p>", "<p>" + (QuizNew.questionCount + 1) + "</p>");
      ...
      newQuestion.find('.image-preview').attr('src', '#').hide();
      ...
      newQuestion.find('.choose-front').addClass('selected');
    ...
    showImagePreview: function() {
      ...
      if (input.files && input.files[0]) {
        ...
        $(this).parent().find('.close-image').show();
        $(this).parents('.card-side').find('.card-color-input').val("");
        return $(this).parents('.card-side').find('.question-image-input').attr('style', "").css('background', 'none');
      }
    },
    closeImage: function() {
      var box;
...
(function() {
  ...
  QuizValidate = {
    init: function() {
      ...
      return $('body').on('click', '#submit-bar .btn', this.submitQuiz);
    },
    ...
    submitQuiz: function() {
      var action, actionText, valid;
      action = $(this).attr('id');
      $(this).parents("#submit-bar").find('.quiz-submit-action').val(action);
      $('.question:visible .item-order').each(function(i) {
        return $(this).val(i + 1);
      });
...
(function() {
  ...
  QuizIndex = {
    init: function() {
      ...
      return this.formatAds();
    },
    formatAds: function() {
      return $('.bottom-ad').each(function() {
        return $(this).find('iframe').attr('width', '180').attr('height', '150');
      });
    },
    loadMore: function() {
      var offset;
